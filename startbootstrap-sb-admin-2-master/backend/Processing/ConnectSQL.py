...
class SqlFunction:
    ...
    def stats_revenue_by_day(self, day):
        try:
            ...
            for r in cursor:
                ans.append('{:.02f}%'.format(float(r[1]) / ans[-1] * 100))
                break
            cursor.commit()
            if len(ans) == 0:
                ans.append(day)
...
sql_func.delete_function("UserLogin", "staff_id", 'NV008')
sql_func.delete_function("Staff", "id", 'NV008')
sql_func.delete_function("Person", "id", 'NV008')